...
docker run -v $(pwd)"/test/zip/":/test/zip/ --rm -ti wikitolearn/ocg:$(cat new_docker_image_version) rm "$OUTPUT"

docker run -v $(pwd)"/test/zip/":/test/zip/ --rm -ti wikitolearn/ocg:$(cat new_docker_image_version) \
